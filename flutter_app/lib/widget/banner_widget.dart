...
class BannerWidget extends StatefulWidget {
  ...
  const BannerWidget(this.dataList, this.indicatorColor, {super.key});
  ...
  Widget build(BuildContext context) {
    return Listener(
      onPointerDown: (PointerDownEvent event) {
        debugPrint('鼠标按下、手指按下，停止自动轮播');
        _stopAutoScroll();
      },
      onPointerUp: (PointerUpEvent event) {
        debugPrint(' 鼠标松开、手指松开，开始自动轮播');
        _startAutoScroll();
      },
      onPointerCancel: (PointerCancelEvent event) {
        debugPrint(' 不是由用户触发的，而是系统经过逻辑判断后对某个 Widget 发送取消事件，开始自动轮播');
        _startAutoScroll();
      },
      child: Stack(
        children: [
